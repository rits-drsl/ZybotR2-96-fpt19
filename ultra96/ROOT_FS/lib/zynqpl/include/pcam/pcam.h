/**
 *  PcamController: Pcamの初期化処理を行うクラス
 *
...
#ifndef SRC_PCAMCONTROLLER_H_
#define SRC_PCAMCONTROLLER_H_
...
namespace zynqpl {
    namespace OV5640_cfg {
        ...
        typedef struct { mode_t mode; config_word_t const* cfg; size_t cfg_size; } config_modes_t;
        ...
        typedef enum { MODE_720P_1280_720_60fps = 0, MODE_1080P_1920_1080_15fps, MODE_1080P_1920_1080_30fps, MODE_END } mode_t;
        typedef struct { uint16_t width; uint16_t height; } mode_resolution_t;
        ...
        config_word_t const cfg_720p_60fps_[] = {
            ...
            //[2]=0 ISP mirror, [1]=1 sensor mirror, [0]=1 horizontal binning
            {0x3821, 0x06},
        ...
        config_word_t const cfg_init_[] = {
            ...
            // PLL1 configuration
            // [7:4]=0001 System clock divider /1,
            // [3:0]=0001 Scale divider for MIPI /1
            {0x3035, 0x11},
            // [7:0]=56 PLL multiplier
            {0x3036, 0x38},
            // [4]=1 PLL root divider /2,
            // [3:0]=1 PLL pre-divider /1
            {0x3037, 0x11},
            // [5:4]=00 PCLK root divider /1,
            // [3:2]=00 SCLK2x root divider /1,
            // [1:0]=01 SCLK root divider /2
            {0x3108, 0x01},
            // PLL2 configuration
            // [5:4]=01 PRE_DIV_SP /1.5,
            // [2]=1 R_DIV_SP /1,
            // [1:0]=00 DIV12_SP /1
            {0x303D, 0x10},
            ...
            // [7]=1 LENC correction enabled,
            // [5]=1 RAW gamma enabled,
            // [2]=1 Black pixel cancellation enabled,
            // [1]=1 White pixel cancellation enabled,
            // [0]=1 Color interpolation enabled
            {0x5000, 0x07},

            // [7]=0 Special digital effects,
            // [5]=0 scaling,
            // [2]=0 UV average disabled,
            // [1]=1 Color matrix enabled,
            // [0]=1 Auto white balance enabled
            {0x5001, 0x03}
        };
        ...
        mode_resolution_t const resolutions[] = {
            { 1280, 720  },
            { 1920, 1080 },
            { 1920, 1080 },
        };

        const uint8_t  OV5640_SLAVE_ADDR = 0x78;
        const uint8_t  DEV_ID_H_         = 0x56;
        const uint8_t  DEV_ID_L_         = 0x40;
        const uint16_t REG_ID_H          = 0x300A;
        const uint16_t REG_ID_L          = 0x300B;
    }
    ...
    public:
        /**
         *  Pcam(Constructor)
         *  @iic_devname:   /dev/下にあるiicデバイスファイル名
         *  @video_devname: /dev/下にあるvideoデバイスファイル名
         *  @mode:          解像度・FPSの設定
         *  @awb:           Auto White Balanceの設定
         *  @pixelformat:   V4L2の画像フォーマット(/usr/include/linux/videodev2.hに定義されている)
         */
        Pcam(const std::string& video_devname,
             const std::string& iic_devname,
             OV5640_cfg::mode_t mode         = OV5640_cfg::mode_t::MODE_720P_1280_720_60fps,
             OV5640_cfg::awb_t  awb          = OV5640_cfg::awb_t::AWB_ADVANCED,
             uint32_t           pixelformat  = V4L2_PIX_FMT_GREY);
        ~Pcam();
         ...
         *  @Return: 画像データの先頭ポインタ
         */
        void fetchFrame(uint8_t* frame) const;
    ...
    private:
        ...
        void shutdown() const;
        void reset() const;
        void applyMode(OV5640_cfg::mode_t mode) const;
        void applyAwb(OV5640_cfg::awb_t awb) const;

        void readReg(uint16_t reg_addr, uint8_t& buf) const;
        void writeReg(uint16_t reg_addr, uint8_t const reg_data) const;
        void writeConfig(OV5640_cfg::config_word_t const* cfg, size_t cfg_size) const;

    private:
        std::unique_ptr<zynqpl::PSGPIO>          psgpio_;
        std::unique_ptr<zynqpl::PSIIC>           psiic_;
        std::unique_ptr<zynqpl::VideoController> video_;
        uint8_t* v4l2_buf_;
        uint32_t width_, height_;
    };
}

#endif /* SRC_PCAMCONTROLLER_H_ */
